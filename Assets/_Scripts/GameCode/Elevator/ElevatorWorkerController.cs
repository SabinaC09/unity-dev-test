...
namespace GameCode.Elevator
{
    public class ElevatorWorkerController
    {
        ...
        private readonly MineshaftPool _mineshaftPool;
        ...
        public ElevatorWorkerController(ElevatorView elevatorView, ElevatorModel elevatorModel, WorkerModel workerModel,
             MineshaftPool mineshaftPool, CompositeDisposable disposable)
        {
            _elevatorView = elevatorView;
            _elevatorModel = elevatorModel;
            _workerModel = workerModel;
            _mineshaftPool = mineshaftPool;
        ...
        private Vector2 GetCurrentTargetPosition()
        {
            switch (_workerModel.State)
            {
                case WorkerState.LOAD:
                    return _mineshaftPool.GetView(_targetMineshaftNumber).Positions.ElevatorLoadPosition;
        ...
        private void TriggerWorking(bool working)
        {
            ...
            if (!working)
            {
                if(_workerModel.State == WorkerState.LOAD && (_workerModel.CarryingAmount.Value >= _workerModel.CarryingCapacity.Value || _targetMineshaftNumber >= _mineshaftPool.GetCount()))
                {
        ...
        {
            return _mineshaftPool.GetModel(_targetMineshaftNumber).DrawResource(amount);
        }
    }
}